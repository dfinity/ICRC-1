...
The textual representation of accounts with non-default subaccounts consists of three parts:
  1. The textual encoding of the account owner as described in the [Internet Computer Interface Specification](https://internetcomputer.org/docs/current/references/ic-interface-spec#textual-ids).
  2. The CRC-32 checksum of concatenated bytes of the principal (up to 29 bytes) and the subaccount (32 bytes).
  3. The hex-encoded bytes of the subaccount with all the leading '0' characters removed.
